...
public interface UserNeo4JRepository extends Neo4jRepository<UserNeo4J, String> {
    ...
    @Query("MATCH (u:User)-[r:MOVIE_LIKE]->(m:Movie) WHERE u.username = $userId ORDER BY r.date DESC RETURN m.title AS title, m.imdb_id AS movieId, m.Poster_path AS poster ")
    List<UserMovie> getLikedMovies(String userId);
    ...
    @Query("MATCH (u:User)-[r:ACTOR_FOLLOW]->(m:Celebrity) WHERE u.username = $userId ORDER BY r.date DESC RETURN m.name AS name, m.person_id AS celebrityId ")
    List<UserCelebrity> getFollowedCelebrities(String userId);
    ...
    @Query("MATCH (u:User)-[r:ACTOR_FOLLOW]->(m:Celebrity) WHERE u.username = $userId AND m.person_id = $celebrityId DELETE r WITH u " +
            "MATCH (u)-[r2:ACTOR_FOLLOW]->(m2:Celebrity) ORDER BY r2.date DESC LIMIT 5 RETURN m2.name AS name, m2.person_id AS celebrityId ")
    List<UserCelebrity> removeFromFollowedCelebrities(String userId, int celebrityId);

    @Query("MATCH (u:User)-[r:FOLLOW]->(m:User) WHERE u.username = $userId RETURN m.username AS username, m.name AS name, m.surname AS surname, m.country AS country, m.favorite_genres AS favorite_genres ")
    List<UserNeo4J> getFollowed(String userId);

    @Query("MATCH (u:User)<-[r:FOLLOW]-(m:User) WHERE u.username = $userId RETURN m.username AS username, m.name AS name, m.surname AS surname, m.country AS country, m.favorite_genres AS favorite_genres ")
    List<UserNeo4J> getFollowers(String userId);
    ...
    @Query("MATCH (user:User {username:$userId})-[:FOLLOW|ACTOR_FOLLOW]->(intermediate)-[:ACTED_IN|DIRECTED_IN|MOVIE_LIKE]->(recommendedMovie:Movie) " +
            ...
            "LIMIT 100")
    List<MovieRecommendationsDTO> recommendMovies(String userId);
    ...
    @Query("MATCH (user:User {username: $userId})-[:FOLLOW|MOVIE_LIKE]->(intermediate)-[:ACTOR_FOLLOW|DIRECTED_IN|ACTED_IN]-(recommendedCelebrity:Celebrity) " +
            ...
            "LIMIT 100")
    List<CelebrityRecommendationsDTO> recommendCelebrities(String userId);
    ...
    @Query("MATCH (user:User {username: $userId})-[:FOLLOW|ACTOR_FOLLOW|MOVIE_LIKE]->(intermediate)-[:FOLLOW|ACTOR_FOLLOW|MOVIE_LIKE]-(recommendedUser:User) " +
            ...
            "ORDER BY numRelations DESC, FollowerCount DESC " +
            "LIMIT 100")
    List<UserRecommendationsDTO> recommendUsers(String userId);
    ...
    @Query("MATCH (user:User {username: $userId})-[:REVIEW_LIKE]->(Review)<-[:POSTED]-(User)-[:POSTED]->(r:Review)-[:EVALUATES]->(recommendedMovie:Movie) " +
            ...
            "ORDER BY (0.5*commonGenres + numRelations) DESC, LikeCount DESC " +
            "LIMIT 100")
    List<MovieRecommendationsDTO> recommendByReview(String userId);
    ...
    @Query("MATCH (user:User {username:$userId})-[:MOVIE_LIKE]->(likedMovie:Movie)<-[:ACTED_IN]-(intermediate:Celebrity)-[:ACTED_IN]->(recommendedMovie:Movie) " +
            ...
            "ORDER BY (0.5*commonGenres + numRelations) DESC, LikeCount DESC " +
            "LIMIT 100")
    List<MovieRecommendationsDTO> recommendByCast(String userId);
    ...
    void updateUser(String username, String name, String surname, String country, List<GenreEnum> favoriteGenres);
    ...
    boolean isCelebrityFollowed(String username, int celebrityId);
